...
if ($module=='halamanstatis' AND $act=='hapus'){
  ...
  if ($data['gambar']!=''){
     ...
     unlink("../../../images/statis/$_GET[namafile]");   
     unlink("../../../images/statis/small_$_GET[namafile]");   
  }
  else{
     mysql_query("DELETE FROM halamanstatis WHERE id_halaman='$_GET[id]'");
  }
...
$data=mysql_fetch_array(mysql_query("SELECT gambar FROM halamanstatis WHERE id_halaman=$num"));
  if ($data['gambar']!=''){
     unlink("../../../images/statis/$data[gambar]");   
     unlink("../../../images/statis/small_$data[gambar]");   
} 
mysql_query("DELETE FROM halamanstatis WHERE id_halaman=$num");
  header('location:../../media.php?module='.$module.'&msg=delete');
}
} else {
  header('location:../../media.php?module='.$module.'&msg=delete');
...
elseif ($module=='halamanstatis' AND $act=='update'){
  ...
  if (empty($lokasi_file)){
   ...
   $data_gambar = mysql_query("SELECT gambar FROM halamanstatis WHERE id_halaman='$_POST[id]'");
	...
	@unlink('../../../images/statis/'.$r['gambar']);
	@unlink('../../../images/statis/'.'small_'.$r['gambar']);
    UploadStatis($nama_file_unik ,'../../../images/statis/');
    mysql_query("UPDATE halamanstatis SET judul        = '$_POST[judul]',
                                          judul_seo    = '$judul_seo',
                                          isi_halaman  = '$_POST[isi_halaman]',
